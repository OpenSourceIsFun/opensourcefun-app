...
export interface DistributionDatesFormValues {
  startDate: Date | null;
  endDate: Date | null;
}

export const useSetDistributionRound = (
  project: Project,
  callback?: () => void,
) => {
  ...
  const { state, send, resetState } = useContractFunction(
    ...
    {
      transactionName: 'setDistributionRound',
    },
  );

  const setDistributionRound = useCallback(
    async ({ startDate, endDate }: DistributionDatesFormValues) => {
      setLoading(true);
  ...
  useEffect(() => {
    (async () => {
      if (loading && state.status === 'Success') {
        ...
        resetState();
        setLoading(false);
        toast.success('Distribution dates updated successfully');
      }
  ...
  return { loading, setDistributionRound };
};
